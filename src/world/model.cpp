...
void cg::world::model::load_obj(const std::filesystem::path& model_path)
{
	...
	std::vector<std::map<std::tuple<int, int, int>, unsigned int>> per_shape_index_map(shapes.size());
	...
	for (size_t s = 0; s < shapes.size(); s++)
	{
		...
		for (size_t f = 0; f < shapes[s].mesh.num_face_vertices.size(); f++)
		{
			for (size_t v = 0; v < shapes[s].mesh.num_face_vertices[f]; v++)
			{
				...
				if (per_shape_index_map[s].count(idx_tuple) == 0)
				{
					per_shape_index_map[s][idx_tuple] = per_shape_vertex_size;
		...
		per_shape_index_buffer.push_back(
			std::make_shared<cg::resource<unsigned int>>(per_shape_index_size));

		per_shape_texture_files.push_back(
		...
		{
			per_shape_texture_files[s] = std::filesystem::path();
		}
	}
	...
	index_map.clear();
	per_shape_index_map.clear();
	per_shape_index_map.resize(shapes.size());
		...
		for (size_t f = 0; f < shapes[s].mesh.num_face_vertices.size(); f++)
		{
			...
			for (size_t v = 0; v < fv; v++)
			{
				...
				if (per_shape_index_map[s].count(idx_tuple) == 0)
				{
					...
					per_shape_index_map[s][idx_tuple] = per_shape_id;
					per_shape_id++;
				}

				per_shape_index_buffer[s]->item(per_shape_index_id) =
					per_shape_index_map[s][idx_tuple];
				per_shape_index_id++;
			}
			index_offset += fv;
...
std::vector<std::shared_ptr<cg::resource<unsigned int>>>
	cg::world::model::get_per_shape_index_buffer() const
{
	return per_shape_index_buffer;
}

std::vector<std::filesystem::path> cg::world::model::get_per_shape_texture_files() const
{
	return per_shape_texture_files;
}
